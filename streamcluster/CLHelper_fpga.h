...
char kernel_file[100]  = "./binary/streamcluster_kernels.xclbin";
int total_kernels = 2;
...
void _clInit(string device_type, int device_id)throw(string){
    ...
    // Select the target FPGA platform
   ...
   //--cambine-4: Create an OpenCL command queue    
    ...
    char *krnl_bin;
    const size_t krnl_size = load_file_to_memory(kernel_file, &krnl_bin);

    oclHandles.program = clCreateProgramWithBinary(oclHandles.context, 1,
	                             oclHandles.devices, &krnl_size,
