...
void OpenCL::buildKernel(int use_device)
{
        if (use_device < 2) {
	    ...
	    free(source_str);
            ...
            char * krnl_file = "./binary/pathfinder_dynproc_kernel.xclbin";
            char *krnl_bin;
...
void OpenCL::getDevices(cl_device_type deviceType)
{
	...
	clGetDeviceIDs(platform_id[use_platform], deviceType, 100, device_id, &devices_n);
	if (VERBOSE)
	{
