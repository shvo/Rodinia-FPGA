...
int main(int argc, char** argv)
{
  ...
  int size_simple = (numCols+2) * (numRows+2) * (layers+2);  
  float*        tIn      = (float*) calloc(size_simple,sizeof(float));
  ...
  int i                 = 0;
  int count = size;
  int count_simple = size_simple;

  int ii;
  //readinput(tIn,numRows, numCols, layers, tfile);
  for (ii = 0; ii < size_simple; ++ii) {
      tIn[ii] = 1;
  }
  for (ii = 0; ii < size; ++ii) {
      tempCopy[ii] = 1;
  ...
  //memcpy(tempCopy,tIn, size_simple * sizeof(float));
  ...
  cl_mem d_b;                     
  cl_mem d_c;                     
  const char *KernelSource = load_kernel_source("hotspotKernel_simple.cl"); 
  ...
  d_a  = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float) * count_simple, NULL, NULL);
  ...
  err = clEnqueueWriteBuffer(commands, d_a, CL_TRUE, 0, sizeof(float) * count_simple, tIn, 0, NULL, NULL);
  if (err != CL_SUCCESS)
    {
